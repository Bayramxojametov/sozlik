...
import { useGetByIdWord, useUpdateWord } from "../../services/words/words.api";
import { useEffect } from "react";

export const UpdateWordsForm = ({ id }) => {
  const [form] = Form.useForm();
  const navigate = useNavigate();
  const { data } = useGetByIdWord(id);

  useEffect(() => {
    if (data) {
      const example_ids_ru = data.data.examples.map((item) => item.text_ru);
      const example_ids_en = data.data.examples.map((item) => item.text_en);

      form.setFieldsValue({
        word_ru: data.word_ru,
        word_en: data.data.word_en,
        category_id: data.data.category.id,
        descriptions_ru: data.data.descriptions_ru,
        descriptions_en: data.data.descriptions_en,
        example_ids_ru: example_ids_ru,
        example_ids_en: example_ids_en,
      });
    }
  }, [data, id]);
  ...
  return (
    <Form form={form} onFinish={onFinish} layout="vertical" name="add-word">
      ...
      <Form.Item label={null}>
        <Button loading={isUpdating} htmlType="submit" type="primary">
          Изменить
        </Button>
      </Form.Item>
    </Form>
